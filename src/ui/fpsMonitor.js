/**
 * fpsMonitor.js — Lightweight FPS/frame-time overlay for debug mode.
 *
 ...
 */
export function showFpsMonitor() {
 ...
 */
export function hideFpsMonitor() {
  if (!_container) return;
  _container.style.display = 'none';
  _active = false;
}
 ...
 *
 * @param {number} frameTimeMs  Delta time for this frame in milliseconds.
 * @param {number} timestamp    performance.now() value from rAF.
 */
export function recordFrame(frameTimeMs, timestamp) {
 ...
 */
export function destroyFpsMonitor() {
  if (_container) {
    _container.remove();
    _container = null;
  }
 ...
 * @param {number} minFt  Minimum frame time across the history buffer.
 * @param {number} maxFt  Maximum frame time across the history buffer.
 */
function _drawGraph(minFt, maxFt) {
