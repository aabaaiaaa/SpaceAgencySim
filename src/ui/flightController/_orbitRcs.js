/**
 * _orbitRcs.js — Normal orbit RCS thrust application, docking mode toggle,
 * RCS mode toggle.
 *
 ...
 */
export function applyNormalOrbitRcs() {
  const s = getFCState();
  ...
  const nBodyId = (s.flightState && s.flightState.bodyId) || 'EARTH';
 ...
 */
export function toggleDockingMode() {
  const s = getFCState();
  ...
  if (s.ps.controlMode === ControlMode.DOCKING || s.ps.controlMode === ControlMode.RCS) {
    ...
    const dockBodyId = (s.flightState && s.flightState.bodyId) || 'EARTH';
 ...
 */
export function toggleRcsModeHandler() {
  const s = getFCState();
