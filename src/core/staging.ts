...
// Debris ID counter
// ---------------------------------------------------------------------------
// The counter, its reset function, and the DebrisState constructor now live
// in `./debris.ts`. Re-exported here so existing consumers of `staging.ts`
// continue to work unchanged.

import { createDebrisFromParts, resetDebrisIdCounter } from './debris.ts';
  ...
  for (const instanceId of instanceIds) {
    ...
    switch (behaviour) {
      ...
      case 'SEPARATE': {
        ...
        // from both sides).  Re-add temporarily so createDebrisFromParts
        // can transfer it properly.
        ps.activeParts.add(instanceId);
        const decouplerDebris = createDebrisFromParts(ps, [instanceId], assembly);
      ...
      case 'RELEASE': {
        ...
        const satelliteDebris = createDebrisFromParts(ps, [instanceId], assembly);
...
export function activatePartDirect(
  ...
  switch (behaviour) {
    ...
    case 'SEPARATE': {
      ...
      ps.activeParts.add(instanceId);
      const directDecDebris = createDebrisFromParts(ps, [instanceId], assembly);
    ...
    case 'RELEASE': {
      ...
      const directSatDebris = createDebrisFromParts(ps, [instanceId], assembly);
...
export function recomputeActiveGraph(ps: PhysicsState, assembly: RocketAssembly): DebrisState[] {
  ...
  const debris = createDebrisFromParts(ps, disconnected, assembly);
  return [debris];
}
