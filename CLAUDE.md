...
- **Physics-worker serialise contract** — `PhysicsSnapshot` crosses the main↔worker boundary via **two separate serialisers** (`src/ui/flightController/_workerBridge.ts::_serialisePhysicsState` for main→worker, `src/core/physicsWorker.ts::serialisePhysicsState` for worker→main) plus a shared `deserialisePhysicsState`. Adding a field requires updating all three — silent drift here has repeatedly caused "feature works in unit tests, does nothing at runtime" bugs. Each site has a ⚠️ comment linking to the others. Round-trip the change in a test that goes `ps → bridge serialise → worker deserialise → tick → assert` (see `src/tests/fuelsystem.test.ts` for the pattern).
