...
async function seedAndFly(page: Page): Promise<void> {
  await page.setViewportSize({ width: VP_W, height: VP_H });
  await seedAndLoadSave(page, makeEnvelope());
  await startTestFlight(page, FLIGHT_PARTS);
}

async function seedBuildAndLaunch(page: Page): Promise<void> {
  await page.setViewportSize({ width: VP_W, height: VP_H });
  await seedAndLoadSave(page, makeEnvelope());
...
test.describe('Flight — Launch & Basic Flight', () => {
  ...
  test('(3) at launch (before staging), altitude is near 0 m', async ({ page }) => {
    ...
    const posY: number = await page.evaluate((): number => window.__flightPs?.posY ?? -1);
    expect(posY).toBeGreaterThanOrEqual(0);
    expect(posY).toBeLessThan(5);

    const altText: string | null = await page.locator('#hud-alt').textContent();
    expect(altText).toBeTruthy();
    const altNum: number = parseFloat((altText ?? '').replace(/[^0-9.-]/g, ''));
  ...
  test('@smoke (4) pressing spacebar activates Stage 1 — altitude begins increasing', async ({ page }) => {
    ...
    const groundedBefore: boolean = await page.evaluate((): boolean => window.__flightPs?.grounded ?? true);
    expect(groundedBefore).toBe(true);

    await stageAndLiftoff(page);

    const altAfter: number = await page.evaluate((): number => window.__flightPs?.posY ?? 0);
  ...
  test('(5) throttle display reflects keyboard throttle changes (W increases, S decreases)', async ({ page }) => {
    ...
    const initialThrottle: number = await page.evaluate(
      (): number => window.__flightPs?.throttle ?? 1,
    );
  ...
  test('(8) TWR=1 button sets throttle for unit thrust-to-weight ratio; ±0.1 buttons step throttle', async ({ page }) => {
    ...
    const throttleAfterTWR1: number = await page.evaluate((): number => window.__flightPs?.throttle ?? -1);
    expect(throttleAfterTWR1).toBeGreaterThan(0);
    expect(throttleAfterTWR1).toBeLessThan(1);

    const expectedPct: number = Math.round(throttleAfterTWR1 * 100);
    await expect(page.locator('#flight-hud-throttle-pct')).toContainText(
      `${expectedPct}%`,
      { timeout: 2_000 },
    );
    ...
    const throttleAfterPlus: number = await page.evaluate((): number => window.__flightPs?.throttle ?? -1);
    const expectedAfterPlus: number = Math.min(1, Math.round((throttleAfterTWR1 + 0.1) * 10) / 10);
    ...
    const throttleAfterMinus: number = await page.evaluate((): number => window.__flightPs?.throttle ?? -1);
    const expectedAfterMinus: number = Math.round((expectedAfterPlus - 0.1) * 10) / 10;
  ...
  test('(9) clicking "Restart from Launch" resets the flight to the launch pad', async ({ page }) => {
    ...
    const altBefore: number = await page.evaluate((): number => window.__flightPs?.posY ?? 0);
    ...
    const grounded: boolean = await page.evaluate((): boolean => window.__flightPs?.grounded ?? false);
